...
function AuthScreen() {
  ...
  async function handleEmail(e){
    ...
    if(mode==="magic"){
      ...
      if(error)setErr(error.message);else setMsg("Check your email for a magic link!");
    }else if(mode==="signup"){
      const{error}=await supabase.auth.signUp({email,password});
      if(error)setErr(error.message);else setMsg("Check your email to confirm your account!");
    }else{
    ...
    <div style={{minHeight:"100vh",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <h1 style={{fontSize:28,fontWeight:700,color:"#37352f",letterSpacing:"-0.02em",marginBottom:8}}>Frame Brief</h1>
          <p style={{color:"#9b9a97",fontSize:14,fontStyle:"italic"}}>Creative production briefs for photographers &amp; videographers</p>
        </div>
        {err&&<div style={{background:"#fff2f2",border:"1px solid #ffc9c9",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#c0392b"}}>{err}</div>}
        {msg&&<div style={{background:"#e6f4ea",border:"1px solid #a8d5b5",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#1e7e34"}}>{msg}</div>}
        <button onClick={handleGoogle} disabled={loading} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:12,padding:"13px 20px",border:"1px solid #e8e4dc",borderRadius:8,background:"#fff",cursor:"pointer",fontFamily:"'Lora',serif",fontSize:14,color:"#37352f",marginBottom:16,transition:"all .15s"}} onMouseEnter={e=>e.currentTarget.style.background="#f7f6f3"} onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}><div style={{flex:1,height:1,background:"#f1f0ef"}}/><span style={{fontSize:12,color:"#c4c3bf"}}>or</span><div style={{flex:1,height:1,background:"#f1f0ef"}}/></div>
        <form onSubmit={handleEmail}>
          <input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="Email address" required style={{width:"100%",border:"1px solid #e8e4dc",borderRadius:8,padding:"12px 14px",fontFamily:"'Lora',serif",fontSize:14,color:"#37352f",outline:"none",marginBottom:10,background:"#fafaf9"}} onFocus={e=>e.target.style.borderColor="#37352f"} onBlur={e=>e.target.style.borderColor="#e8e4dc"}/>
          {mode!=="magic"&&<input value={password} onChange={e=>setPassword(e.target.value)} type="password" placeholder="Password" required style={{width:"100%",border:"1px solid #e8e4dc",borderRadius:8,padding:"12px 14px",fontFamily:"'Lora',serif",fontSize:14,color:"#37352f",outline:"none",marginBottom:10,background:"#fafaf9"}} onFocus={e=>e.target.style.borderColor="#37352f"} onBlur={e=>e.target.style.borderColor="#e8e4dc"}/>}
          <button type="submit" disabled={loading} style={{width:"100%",background:"#37352f",color:"#fff",border:"none",padding:"13px",borderRadius:8,fontFamily:"'Lora',serif",fontSize:14,cursor:"pointer",opacity:loading?0.6:1,marginBottom:14}}>{loading?"Loading…":mode==="login"?"Sign In":mode==="signup"?"Create Account":"Send Magic Link"}</button>
        </form>
        <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"center"}}>
          {mode==="login"&&<><button onClick={()=>{setMode("signup");setErr("");setMsg("");}} style={{background:"none",border:"none",color:"#9b9a97",fontSize:13,cursor:"pointer",fontFamily:"'Lora',serif"}}>Don't have an account? Sign up</button><button onClick={()=>{setMode("magic");setErr("");setMsg("");}} style={{background:"none",border:"none",color:"#9b9a97",fontSize:13,cursor:"pointer",fontFamily:"'Lora',serif"}}>Sign in with magic link instead</button></>}
          {mode==="signup"&&<button onClick={()=>{setMode("login");setErr("");setMsg("");}} style={{background:"none",border:"none",color:"#9b9a97",fontSize:13,cursor:"pointer",fontFamily:"'Lora',serif"}}>Already have an account? Sign in</button>}
          {mode==="magic"&&<button onClick={()=>{setMode("login");setErr("");setMsg("");}} style={{background:"none",border:"none",color:"#9b9a97",fontSize:13,cursor:"pointer",fontFamily:"'Lora',serif"}}>Back to sign in</button>}
        </div>
      </div>
    </div>
  );
...
function PropRow({label,children}){return(<div style={{display:"flex",alignItems:"baseline",padding:"7px 16px",borderBottom:"1px solid #f1f0ef",gap:12}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#9b9a97",minWidth:126,textTransform:"uppercase",letterSpacing:"0.06em",flexShrink:0,paddingTop:2}}>{label}</span><span style={{flex:1,fontSize:14,color:"#37352f"}}>{children}</span></div>);}
function AddBtn({label,onClick}){return(<button onClick={onClick} style={{background:"none",border:"none",color:"#9b9a97",fontSize:13,cursor:"pointer",padding:"4px 0",fontFamily:"inherit",display:"flex",alignItems:"center",gap:4}} onMouseEnter={e=>e.currentTarget.style.color="#37352f"} onMouseLeave={e=>e.currentTarget.style.color="#9b9a97"}>+ {label}</button>);}
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({projects,onOpen,onNew,onDelete,onStatusChange,user,onSignOut}){
